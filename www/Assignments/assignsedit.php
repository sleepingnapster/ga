...
$query_rsAssigns = sprintf("SELECT assign_id, a_sub_id, a_batch, a_type, a_no, a_part, a_title, a_startdate, a_subdate FROM assign WHERE a_sub_id = %s ORDER BY a_type ASC, a_no ASC", GetSQLValueString($colname_rsAssigns, "int"));
...
<html>
                    ...
                    <div class="row">
                        <div class="col-xs-12">
                                <div class="box-header">
                                    <h3 class="box-title"><?php echo $_SESSION['MM_Subids'][$_GET['subid']]['subabbv']." : ".$_SESSION['MM_Subids'][$_GET['subid']]['subname'];?> >> <a href="../Assignments/allassignsummary.php?subid=<?php echo $_GET['subid'];?>" class="btn btn-primary">ALL SUBMISSIONS</a></h3>
                                </div><!-- /.box-header -->
 ...
 <td><a href="../Assignments/assignteacherview.php?assignid=<?php echo $row_rsAssigns['assign_id']."&subid=".$_GET['subid']; ?>"><?php echo $row_rsAssigns['a_title']; ?></a></td>
						...
						echo "<div style='display:none'>".$row_rsAssigns['a_startdate']."</div>".date('D d M',$xxs); }?></td>
						<td><?php if($xxd=strtotime($row_rsAssigns['a_subdate'])){
							echo "<div style='display:none'>".$row_rsAssigns['a_subdate']."</div>".date('D d M',$xxd);}?><?php
$nowww=date_create();
