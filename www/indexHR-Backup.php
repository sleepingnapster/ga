	...
	if(!isset($_SESSION))
{
    ...
    if (empty($token)) {
		...
		*/
		$hostname_GreenAssign3 = "mydbinstance.cksfic05yyly.us-west-2.rds.amazonaws.com";
...
$insertSQL = sprintf("INSERT INTO user (gplus_id,name,rt,email,entrytime) VALUES (%s,%s,%s,%s,now())",
  ...
  $row_RsUid = mysql_fetch_assoc($ResultUid);
  				...
  				$response = 'http://greenassign.com/Signup/';
				//block 1 working 
            }
			else{
				//block 2 working 
				...
				$response = 'http://greenassign.com/custom.php?forceSignup=onreadyTriggerASigninWithForceToGetRt';
				/*
  ...
  $Result1 = mysql_query($insertSQL, $GreenAssign3) or die(mysql_error());
				*/
				...
				//if rt present update it and continue
    ...
    . "WHERE email = %s LIMIT 0, 30 ;", GetSQLValueString($colname_RsStudent, "text"));
									...
									$response = 'http://greenassign.com/CollegeNews.php';
			...
			$i=0;
			  ...
			  mysql_free_result($rsLoginSubs);
				$_SESSION['MM_Subcount']=$i;
					...
					//$response = 'http://greenassign.com/home.php';
					
				}
			}
		
    } else {
        $response = 'http://greenassign.com/CollegeNews.php?token=BCTokenHai';
    }
