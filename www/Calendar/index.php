...
$totalRows_RsMyAssignStatuses = mysql_num_rows($RsMyAssignStatuses);
	...
	$strsubs2 .= $_SESSION['MM_Subids'][$i]['subid'].', ';
	$relativesubid[$_SESSION['MM_Subids'][$i]['subid']] = $i;
	}
	$strsubs2 .= '-7';
	
	mysql_select_db($database_greenassign, $greenassign);
$query_RsTeacherAssigns = "SELECT * FROM assign WHERE a_sub_id IN (".$strsubs2.")";
$RsTeacherAssigns = mysql_query($query_RsTeacherAssigns, $greenassign) or die(mysql_error());
//$row_RsTeacherAssigns = mysql_fetch_assoc($RsTeacherAssigns);
$totalRows_RsTeacherAssigns = mysql_num_rows($RsTeacherAssigns);
...
if(!(isset($_SESSION['GA_usertype'])&&($_SESSION['GA_usertype']=="teacher")&&(isset($_SESSION['GA_ut_id'])))){//NOT a teacher
					  ...
					  while ($row_RsMyAssignStatuses = mysql_fetch_assoc($RsMyAssignStatuses)){ ?>
							...
							url: '../Assignments/assignview.php?subid=<?php echo $row_RsMyAssignStatuses['a_sub_id']; ?>&assignid=<?php echo $row_RsMyAssignStatuses['assign_id']; ?>',
                             ...
                             <?php 
								if($row_RsMyAssignStatuses['subm_status']==""){
									...
									$diff=date_diff(date_create('now'),date_create($row_RsMyAssignStatuses['a_subdate']));
						...
						while ($row_RsTeacherAssigns = mysql_fetch_assoc($RsTeacherAssigns)) { ?>
							{	
                            title: '<?php echo $_SESSION['MM_Subids'][$relativesubid[$row_RsTeacherAssigns['a_sub_id']]]['subabbv']." ".$_SESSION['MM_Subids'][$relativesubid[$row_RsTeacherAssigns['a_sub_id']]]['subclass']." > ". $row_RsTeacherAssigns['a_type']." ".$row_RsTeacherAssigns['a_no']." - ".$row_RsTeacherAssigns['a_title']; ?>',
                            start: new Date("<?php echo $row_RsTeacherAssigns['a_subdate']; ?>"),
							url: '../Assignments/assignteacherview.php?assignid=<?php echo $row_RsTeacherAssigns['assign_id']; ?>&subid=<?php echo $relativesubid[$row_RsTeacherAssigns['a_sub_id']]; ?>',
                        <?php 
								$assignsubdate=date_create($row_RsTeacherAssigns['a_subdate']);
								$diff=date_diff(date_create('now'),date_create($row_RsTeacherAssigns['a_subdate']));
								if($diff->format('%R%a')>0){
					 echo 'backgroundColor: "#0073b7",borderColor: "#0073b7"';/*blue*/ }
				else{echo 'backgroundColor: "#f56954",borderColor: "#f4543c"';/*red*/}
						?> 
...
/*
if(!(isset($_SESSION['GA_usertype'])&&($_SESSION['GA_usertype']=="teacher")&&(isset($_SESSION['GA_ut_id'])))){//NOT a teacher
mysql_free_result($RsTeacherAssigns);

mysql_free_result($RsMyAssignStatuses);}else
