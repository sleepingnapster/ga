...
else{header("Location: ../SelectClass.php?page=resources/eBooks.php");exit;}
mysql_select_db($database_greenassign, $greenassign);
$query_rsClassSubids = sprintf("SELECT sub_id, sub_name, sub_abbv FROM sub WHERE sub_class = %s ORDER BY sub_id ASC", GetSQLValueString($colname_rsClassSubids, "text"));
...
mysql_select_db($database_greenassign, $greenassign);
$query_rsSubidEbooks = sprintf("SELECT eb_id, eb_filename, eb_clean, eb_verified, eb_uploadedby FROM ebooks WHERE eb_subid = %s AND (eb_clean OR eb_verified = 1) ORDER BY eb_id ASC", GetSQLValueString($colname_rsSubid, "text"));
...
<td><a href="../pages/resources/report.php?type=eBooks&file=<?php echo $row_rsSubidEbooks['eb_filename']; ?>">Report</a></td>
                                        ...
                                        <form action="eBooksUpload.php" method="POST" name="form" enctype="multipart/form-data">
<p>
